 ...
 */
const FORMAT_VERSION = 'b6';
const SUPPORTED_VERSIONS = new Set(['b1', 'b2', 'b3', 'b4', 'b5', 'b6']);
...
/**
 * 廢牌堆的進入順序：每張卡各寫一次（同名卡重複寫），不合併張數 ——
 * 合併了就失去順序。
 */
const encodeOrder = (order: string[], byId: Map<string, Card>): string =>
  order
    .map((id) => byId.get(id))
    .filter((card): card is Card => Boolean(card))
    .map((card) => shortCode(card))
    .join('.');

function decodeOrder(encoded: string, index: Map<string, Card>): { order: string[]; dropped: number } {
  const order: string[] = [];
  let dropped = 0;
