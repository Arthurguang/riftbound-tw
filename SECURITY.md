...
> **另一份要一起看的**：[SECURITY-STRATEGY.md](SECURITY-STRATEGY.md) 是**上線前的盤點清單** ——
> 五個要問自己的問題、目前查證出來的現況，以及還沒補的缺口。
> 這份講設計理由，那份講「現在到底做到哪了」。

---

## 本站的安全憲法（Security Invariants）

**這一節是給資安審查用的 —— 人看、AI 掃描工具也看。**

下面六條是本站**任何情況下都不能被打破的鐵律**。做資安審查時，
請直接拿這六條去比對程式碼，找出任何會打破它們的漏洞。

通用的掃描工具找得到通用的漏洞樣式，但**找不到違反這六條的問題** ——
尤其是第四條，那是這個產品獨有的規則。

| # | 鐵律 | 目前靠什麼守 |
|---|---|---|
| 1 | **使用者瀏覽器裡的任何東西永遠不可信**（網址參數、localStorage、sessionStorage、貼上的文字）—— 一律重新驗證，逐項比對允許清單，不認得就丟掉 | `decodeDeck` / `decodeBoard` / `filtersFromParams` / `importDeck`，全部 fail-closed |
| 2 | **外部資料驗不過就中斷建置**，絕不「盡量解析」後放行 | `fetch-cards.mjs` 的 Zod 驗證與 HTML token 解析；`fetch-errata.mjs` 的純文字斷言與卡片對應檢查 |
| 3 | **絕不為了讓功能動而放寬 CSP** —— 功能與 CSP 衝突時改的是程式碼 | 標頭斷言測試（不含 `unsafe-inline` / `unsafe-eval`）＋ 三引擎實測零違規 |
| 4 | **任何規則敘述都必須附官方出處**（規則條號、禁卡表版本、勘誤日期）—— 本站的核心資產是規則正確性，講錯規則就是最大的損害 | `LegalityIssue.rule` 欄位；禁卡與勘誤都標版本與官方連結；測試斷言這些欄位存在 |
| 5 | **不蒐集個資、不用 cookie、不放第三方執行期腳本** | 架構上沒有後端；CSP 為 `default-src 'none'`；端對端測試斷言收藏功能不發網路請求 |
| 6 | **不做對戰系統、不做對局戰績與勝率統計**（Riot 開發者政策，非技術限制） | 復盤頁面有測試釘住「研究工具，不是對戰系統」的定位聲明 |

> 每一條的完整說明、以及它們各自實際擋下過什麼，見
> [SECURITY-STRATEGY.md 第四問](SECURITY-STRATEGY.md)。
